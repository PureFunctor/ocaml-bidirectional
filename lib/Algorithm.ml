...
let rec subtype (gamma : incomplete_t) (a : polytype_t) (b : polytype_t) =
...
and instL (gamma : incomplete_t) (alpha : string) (a : polytype_t) =
  let* _ = WellFormed.check_type gamma a in
  ...
  | Error _ ->
       ...
       *)
      | TFun (a, b) ->
          let alpha' = fresh_name () in
          let beta' = fresh_name () in
...
and instR (gamma : incomplete_t) (a : polytype_t) (alpha : string) =
  let* _ = WellFormed.check_type gamma a in
  let* _ = WellFormed.check_type gamma (TExists alpha) in
  (* InstRSolve
...
and check (gamma : incomplete_t) (e : expr_t) (t : polytype_t) =
...
and synth (gamma : incomplete_t) (e : expr_t) =
...
and synth_app (gamma : incomplete_t) (t : polytype_t) (e : expr_t) =
